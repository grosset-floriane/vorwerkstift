...
(function () {
  ...
  const tabbables = header.querySelectorAll(
    'li.menu-item-has-children > a, a[rel="home"]'
  );
  ...
  function watchKeypress(e) {
    ...
    if (e.shiftKey) {
      ...
      // going forward
      const lastTabbableChildren =
        tabbables[tabbables.length - 1].parentNode.querySelectorAll("a[href]");
      if (
        target === tabbables[tabbables.length - 1] &&
        !tabbables[tabbables.length - 1].classList.contains("focus")
      ) {
        // Force focus to first element in container if the last element is not open
        e.preventDefault();
        tabbables[0].focus();
      } else if (
        target === lastTabbableChildren[lastTabbableChildren.length - 1]
      ) {
  ...
  for (const link of linksWithChildren) {
    ...
    const subMenuSlug = `submenu-${subMenuTitle
      .toLowerCase()
      .replace(/\s/g, "-")}`;
